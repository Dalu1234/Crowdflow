...
class SFsim(omni.ext.IExt):
    ...
    def demo_api_call(self, Sim):
        ...
        if not Sim.rigidbody:
            ...
            if self.instancer_flag:
                ...
                # Wait for prototype prims to exist before creating instancers
                self._wait_for_prototypes(Sim)
    ...
    def _wait_for_prototypes(self, Sim):
        """Ensure prototype prims exist and have children before creating instancers."""
        stage = omni.usd.get_context().get_stage()
        prototype_paths = [Sim.agent_instance_path_bob]
        if Sim.add_jane:
            prototype_paths.append(Sim.agent_instance_path_jane)

        for proto_path in prototype_paths:
            proto_prim = stage.GetPrimAtPath(proto_path)
            if not proto_prim or not proto_prim.IsValid():
                print(f"[siborg.simulate.crowd] Warning: Prototype {proto_path} not found on stage. Instancer may show warnings.")
            elif not proto_prim.GetChildren():
                print(f"[siborg.simulate.crowd] Warning: Prototype {proto_path} has no children (empty). Instancer may show warnings.")
