...
class Simulator(CrowdConfig):
    ...
    def _single_agent_instance(self, agent_pos, nagents, agent_instance_path, instance_path):
        ...
        # Ensure prototypes relationship uses Sdf.Path targets
        point_instancer.CreatePrototypesRel().SetTargets([Sdf.Path(agent_instance_path)])
...
class WarpCrowd(Simulator):
    ...
    def config_mesh(self, points, faces):
        ...
        # Prepare data for Warp Mesh: points must be vec3, indices must be a flattened 1-D int array
        import numpy as np

        # Ensure points are contiguous float32 (Warp will view them as vec3)
        points_np = np.ascontiguousarray(np.asarray(points, dtype=np.float32))

        # Ensure indices are flattened 1-D int32 (Warp requires a flat index buffer)
        faces_np = np.asarray(faces)
        if faces_np.ndim != 1:
            faces_np = faces_np.reshape(-1)
        ...
        self.mesh = wp.Mesh(
            ...
            indices=wp.array(faces_np, dtype=int, device=self.device),
        )
